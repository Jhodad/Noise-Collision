...
public class OverlapBox : MonoBehaviour
{
    ...
    void MyCollisions()
    {
        ...
        UnityEngine.Collider[] hitColliders = Physics.OverlapBox(gameObject.transform.position, transform.localScale / 2, Quaternion.identity, m_LayerMask);
        int i = 0;

        while (i < hitColliders.Length)
        {
            //Output all of the collider names
            Debug.Log("I'm hitting: " + hitColliders[i].name + " - I: " + i );
